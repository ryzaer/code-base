...
function send_json_response($code,$data){
    ...
    echo json_encode(['status' => $code,"result"=>$data]);
}
if (isset($_GET['id']) && !empty($_GET['id'])){
    ...
    $output = false;
    if($n <= 8 && isset($wil[$n][0])){

        $output = true;
        $query = $db->prepare("SELECT kode,nama FROM indo_wilayah.provinsi WHERE LEFT(kode,:n)=:id AND CHAR_LENGTH(kode)=:m ORDER BY nama");
    ...
    if($n ==13){
        // pencarian id keluarahan 
        $output = true;
        $query = $db->prepare("SELECT * FROM indo_wilayah.provinsi WHERE CHAR_LENGTH(kode)=13 AND kode=:id ORDER BY nama");
        $query->execute(array(':id'=>$_GET['id']));
        $data = $query->fetchObject();
        if($data){
            send_json_response(200,$data->nama);
        }else{
            send_json_response(404,[]);
        }
    }
