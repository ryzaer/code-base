...
class imager {
  ...
  public function overlay($overlay, $anchor = 'center', $opacity = 1, $xOffset = 0, $yOffset = 0, $calculateOffsetFromEdge = false) {
    // Load overlay image
    if(!($overlay instanceof imager)) $overlay = new imager($overlay);
  ...
  public function text($text, $options, &$boundary = null) {
	...
	if(!function_exists('set_text_options')){
		function set_text_options($arrs){
			$setts = [];
			...
			foreach ([
				...
				$setts[$key] = isset($arrs[$key]) ? $arrs[$key] : $value;
			}
			return $setts;
		}
	}
