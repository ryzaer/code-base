...
class HTML2PDF extends \setasign\FPDF
{
...
function TableHtml($data){
    $Wmargin  = '';
    $fullRow  = array_sum($this->tableRows);
    foreach ($data as $key => $value) 
    {
        $getX = $this->GetY();
        $rows = 11;
        $line = $rows - 1;
        $revert = $this->w - $fullRow ;
        $this->yAside = [];
        foreach ($value as $k => $v) {
            $this->SetLeftMargin($rows);
            $rows += $this->tableRows[$k] + 1;            
            $revert = $this->w - $rows;
            $this->SetRightMargin($revert);
            $this->SetY($getX+2); // add 2 for padding
            $this->WriteHTML($v);   
            $this->yAside[] = $this->GetY();            
        }
        
        for ($i=0; $i < count($value) ; $i++) { 
            $this->Line($line,$getX+1,$rows-1,$getX+1);
            $line += $this->tableRows[$i];
        }
        $this->SetY(max($this->yAside)+5);
    }
}
